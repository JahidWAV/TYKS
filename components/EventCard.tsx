...
export default function EventCard({ event }: { event: IortiEvent }) {
  return (
    <article className="group rounded-2xl overflow-hidden border border-surface-hair bg-surface transition-all duration-300 hover:-translate-y-0.5 hover:border-white/15">
      <div className="relative h-56 overflow-hidden bg-surface-raised">
        {event.image_url ? (
          <img
            src={event.image_url}
            alt={event.title}
      ...
      <div className="px-4 py-3.5 flex items-center justify-between">
        <span className="text-base font-bold text-ink">{formatPrice(event.price_cents, event.currency)}</span>
        <button
          disabled
          title="La billetterie en ligne arrive bientôt"
          className="flex items-center gap-1.5 text-ink-faint text-xs font-semibold cursor-not-allowed"
        >
          <span>Bientôt</span>
